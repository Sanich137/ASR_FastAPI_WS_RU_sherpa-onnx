...
async def get_cupy_array_samples_float32(audio_bytes: bytes, sample_width: int = 2) -> cp.ndarray:
    """
    Преобразует аудио в байтах в массив float32 с использованием CuPy.
    """
    # Проверка входных данных
    if not audio_bytes:
        logger.error("Ошибка: audio_bytes пустой")
    ...
    # Определяем тип данных на основе sample_width
    dtype = cp.int16 if sample_width == 2 else cp.int32

    try:
        # Преобразуем байты в массив CuPy
        samples = cp.frombuffer(audio_bytes, dtype=dtype)
        logger.debug(f"Длина samples: {len(samples)}, dtype: {samples.dtype}")

        if len(samples) == 0:
            logger.error("Ошибка: samples пустой после cp.frombuffer")
            raise ValueError("Не удалось преобразовать audio_bytes в массив")

        # Преобразуем в float32
        samples_float32 = samples.astype(cp.float32)
        ...
        # Проверка на NaN и бесконечные значения
        if cp.any(cp.isnan(samples_float32)) or cp.any(cp.isinf(samples_float32)):
            logger.error("Обнаружены NaN или бесконечные значения в samples_float32")
            raise ValueError("Некорректные значения в samples_float32")

        # Проверка диапазона значений
        if cp.max(cp.abs(samples_float32)) > 1.0:
            logger.warning(f"Значения samples_float32 вне диапазона [-1, 1]: max={cp.max(cp.abs(samples_float32))}")
            samples_float32 = cp.clip(samples_float32, -1.0, 1.0)

        logger.debug(
            f"Длина samples_float32: {len(samples_float32)}, min={cp.min(samples_float32)}, max={cp.max(samples_float32)}"
        )
        return samples_float32

    except Exception as e:
        logger.error(f"Ошибка в get_cupy_array_samples_float32: {e}")
        raise
