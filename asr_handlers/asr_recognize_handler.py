...
async def asr_recognize_handler(data: ProcessingState, recognizer: sherpa_onnx.OfflineRecognizer) -> ProcessingState:
    params = data.params
    logger.info(f'Получено задание в asr_recognize_handler')
    audio_to_asr = data.stage_results.audio_to_asr

    for n_channel, audio_asr_list in enumerate(audio_to_asr):
        ...
        for audio_asr in audio_asr_list:
            ...
            try:
                ...
                if params.do_auto_speech_speed_correction or params.speech_speed_correction_multiplier != 1:
                    ...
                    asr_result_wo_conf = await simple_recognise(audio_asr, recognizer)
                ...
                if MODEL_NAME == "Gigaam" or MODEL_NAME == "Gigaam_rnnt":  # Whisper
                    ...
                    asr_result =  await process_asr_json(asr_result_wo_conf, audio_duration)

                data.results.raw_data[f"channel_{n_channel + 1}"].append(asr_result)

                audio_duration += audio_asr.duration_seconds
